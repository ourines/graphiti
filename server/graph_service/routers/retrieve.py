...
async def search(query: SearchQuery, graphiti: ZepGraphitiDep):
    # Search with potentially more results for time filtering
    search_limit = query.max_facts * 3 if (query.start_time or query.end_time) else query.max_facts
    ...
    if query.start_time or query.end_time:
        ...
        facts = filtered_facts[:query.max_facts]
...
async def get_graph_stats(
    ...
    """
    episodes_result, _, _ = await graphiti.driver.execute_query(
        episodes_query,
        group_id=group_id
    )
    episodes_count = episodes_result[0]['count'] if episodes_result else 0
    oldest_memory = episodes_result[0]['oldest'] if episodes_result and episodes_result[0]['oldest'] else None
    newest_memory = episodes_result[0]['newest'] if episodes_result and episodes_result[0]['newest'] else None
    ...
    """
    entities_result, _, _ = await graphiti.driver.execute_query(
        entities_query,
        group_id=group_id
    )
    ...
    """
    facts_result, _, _ = await graphiti.driver.execute_query(
        facts_query,
        group_id=group_id
    )
